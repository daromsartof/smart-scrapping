...
async def main(): 
    try:
        scraping_tool = create_scraping_tool({"max_scrape_length": 20000})
        user_prompt = "lister tous les biens en encore en vente dans cette site web : https://gary.ch/acheter/"
        ...
        # Set up memory to store conversation and scraped content
        memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True)

        # Register the scraping tool for the agent
        tools = [
            Tool(
                name=scraping_tool.name,
                func=scraping_tool._run,
                description=scraping_tool.description,
                coroutine=scraping_tool._arun,
            )
        ]
        ...
        # Let the agent handle the user prompt end-to-end
